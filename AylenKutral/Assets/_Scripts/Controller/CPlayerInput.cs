...
public class CPlayerInput : MonoBehaviour 
{
	...
	public delegate void SpiritAbilityDelegate();
	public static event SpiritAbilityDelegate OnSpiritAbility;
	#endregion
	...
	public GameObject anchimallenPosition;
	...
	void Update () 
	{
		...
		if(cPlayerController.eInputMode != EInputMode.INDIALOG || GameStateManager.eGameState != EGameState.CUTSCENE)
		{

		
			#region "For events"

			if(player.GetButtonDown("UseSpirit"))
			{
				if(OnSpiritAbility!=null)
					OnSpiritAbility();
			}
			#endregion
			...
			if(cPlayerController.eInputMode == EInputMode.FREEMOVEMENT)
			{
				...
				!jump && !onLadder && !cPlayerController.m_OnWater && cPlayerController.m_Grounded)
				{
					//runSpeed = 1.5f;
					print("ingreso true");
					_cPlayerAnimation.ChangeAnimation(EPlayerAnimationState.CROUCH ,"OnCrouch");
					crouch = true;
					anchimallenPosition.transform.position += new Vector3(1,-1,0);
				}
			...
			if (currentInteractiveObject == null && !_onHide && !cPlayerController.m_OnWater)
			{
				...
				{
					print("entro porque el hit colisiono");
			...
			if(onDie)
			{
				_cPlayerAnimation.ChangeAnimation(EPlayerAnimationState.WALK, "OnFlyDie");
			}
			...
			else if(currentInteractiveObject == null)
			{
				...
				&& !onLadder )
				{
					if(!crouch && !cPlayerController.m_OnWater && cPlayerController.m_Grounded )//&& _rb.velocity.y <= 0)
					{
						...
						{
							_cPlayerAnimation.ChangeAnimation(EPlayerAnimationState.IDLE, "Idle");	
				...
				if(!onLadder)
				{
					if(_rb.velocity.y < 0 && !cPlayerController.m_Grounded && !cPlayerController.m_OnWater)
